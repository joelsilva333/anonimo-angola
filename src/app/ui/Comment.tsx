...
interface ReplyInput { text: string; }
...
export default function Comment({
  ...
  const [commentLiked, setCommentLiked] = useState(comment.has_reacted || false);
  ...
  const onSubmit: SubmitHandler<ReplyInput> = async (data) => {
    try {
      setLoading(true);
      const response = await api.post(`/answers/${comment.id}`, { text: data.text });
      refetch();
      if (response.status === 201 && response.data?.answer) {
        setAnswers((prev) =>
          [...prev, { ...response.data.answer, has_reacted: false, likes: 0 }].sort(
            (a: any, b: any) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime(),
          ),
        );
        setShowAnswers(true);
  ...
  const handleCommentLike = async () => {
    ...
    try {
      await api.post(`/reactions/comment/${comment.id}`, { type: prev ? "dislike" : "like" });
      refetch();
    } catch { setCommentLiked(prev); setCommentLikesCount(commentLikesCount); }
  };
  ...
  const handleAnswerLike = async (answerId: string, currentLiked: boolean) => {
    setAnswers((prev) =>
      prev.map((a) =>
        a.id === answerId
          ? { ...a, has_reacted: !currentLiked, like: currentLiked ? (a.like || 1) - 1 : (a.like || 0) + 1 }
          : a,
      ),
    );
    try {
      await api.post(`/reactions/answer/${answerId}`, { type: currentLiked ? "dislike" : "like" });
      refetch();
    } catch {
      setAnswers((prev) =>
        prev.map((a) =>
  ...
  return (
    <li className="w-full flex flex-col gap-2" style={commentStyle}>
      {/* Cabeçalho */}
      <div className="flex gap-2 items-center justify-between">
        <span className="flex gap-2.5 items-center">
          ...
          <span className="flex items-center gap-1.5">
            <p className="text-xs font-semibold text-gray-800">{comment.anon_name}</p>
          ...
          <>
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium transition-all duration-200 cursor-pointer ${
                commentLiked ? "bg-secondary/15 text-secondary" : "hover:bg-black/5 text-gray-500"
              }`}
            >
              <Heart size={12} className={commentLiked ? "fill-current" : ""} />
              {commentLiked ? "Apoiou" : "Apoiar"}
              {commentLikesCount > 0 && <span className="opacity-60">({commentLikesCount})</span>}
            </button>
      ...
      <AnimatePresence>
        {showAnswers && Array.isArray(answers) && answers.length > 0 && (
          ...
          >
            {answers.filter((a) => a && typeof a.text === "string").map((answer) => {
                  ...
                  <div className="flex items-center gap-1.5">
                    {answer.profile_picture && (
                      <Image
                        ...
                        width={20} height={20} unoptimized alt="Perfil"
                        className="rounded-full object-cover w-5 h-5"
                      />
                    )}
                    <span className="text-[11px] text-gray-400">
                      {answer.anon_name || "Anônimo"} •{" "}
                      <TimeAgo date={answer.created_at || new Date()} formatter={customFormatter} />
                    </span>
                  </div>
                  <p className="text-xs text-gray-700">{answer.text}</p>
                  {isAuthenticated() && (
                    <button
                      onClick={() => handleAnswerLike(answer.id, isAnswerLiked)}
                      className={`flex items-center gap-1 px-2 py-0.5 rounded-lg text-[11px] font-medium w-fit transition-all duration-200 cursor-pointer ${
                        isAnswerLiked ? "bg-secondary/15 text-secondary" : "hover:bg-black/5 text-gray-400"
                      }`}
                    >
                      <Heart size={10} className={isAnswerLiked ? "fill-current" : ""} />
                      {isAnswerLiked ? "Apoiou" : "Apoiar"}
                      {answerLikes > 0 && <span className="opacity-60">({answerLikes})</span>}
                    </button>
                  )}
                </motion.div>
              );
      ...
      {replyMode && (
        ...
        >
          ...
          <div className="flex justify-end gap-2">
            <button
              type="button"
              onClick={() => { reset(); setReplyMode(false); }}
